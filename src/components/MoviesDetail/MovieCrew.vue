<template>
  <div class="movie-crew">
      <div style='padding:5px 0' class="container">
        <div style='fontSize:22px;borderBottom:5px solid var(--bs-teal);width:90%;margin:0 auto' class='center'>CREW <span style='marginLeft:5px;fontSize:16px;color:green' class='center'><ion-icon style='marginRight:5px;fontSize:8px' name="ellipse"></ion-icon> {{crew.length}}</span></div>
        <div v-if='crew.length>0' class="mc-crews">
          <carousel :responsive="{0:{items:1},528:{items:2},792:{items:3},1056:{items:4},1320:{items:5}}" style='position:relative' :autoHeight="true" :dots="false" :autoplay="false" :nav="false" :items=5 >
            ...
            <card-person :type="'crew'" v-for='p in crew' :key='"crew"+p.id+p.job' :cast='p'/>
...
export default {
  components:{carousel, CardPerson},
  props:{
    crew:Array,
  }
}
